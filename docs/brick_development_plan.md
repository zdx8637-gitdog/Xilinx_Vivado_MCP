...
> **A2 测试方案 v2（2026-08-25 用户定）**：采集与上行解耦——**PL 环形缓冲连续采集（永不停机）**，PS 收到 UART 指令后上传**最新固定 1s 全 8 通道**原始采样值（数据量固定、非参数），传完即断开；新增**保存数据文件 + 8 通道「ADC 原始值 vs 时间」波形图**对账证据（不换算电压）。需求已改版 [B12_a2_requirement_draft.md](../tests/B12_a2_requirement_draft.md)。D-D 从框架缺陷改判为测试协议问题（由需求 v2 的确定性协议根治）；开发流程修复轮（D1/D-B/D-E/D-A/D-C，P2 记录操作超时不执行）**先行**；修复审查通过后**白盒重跑（Agent1c，新架构）→ 白盒审查通过/保守修复后 A2 黑盒**，按序自动推进（用户授权）；仅重大问题停止等用户。
