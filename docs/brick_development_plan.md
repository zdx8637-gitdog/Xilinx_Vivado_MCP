...
10. **最终验收证据必须 solid（用户 2026-08-25 定调）**：交付到用户面前的验收证据必须是用户可**独立复核**的实物/数值/图形（原始字节样本+校验和可外部重算、真实采样值序列可画波形对照、视觉现象可亲眼确认等），不得以"固件自证的状态行/结论行"充当终审证据；机读判定与外部可复核证据两者缺一不可。
...
### B12：数据采集链路（AD7606C-16）

目标：验证完整 PL/PS 数据通路，作为泛化框架第二个验证实例（第一个是 6-LED）。用户定调：**UART 调试先行、TCP 高速后置、白盒黑盒全走、简单工程先闭环**。ADC 由用户选定 **AD7606C-16**（凌智 16Bit@1MSPS_ADC V1.3，8 通道 16 位 1 MSPS SAR，±10V/±5V 量程，CONVST/BUSY 触发接口，J11 连接——引脚映射 FROZEN，物理事实已入板卡包 `boards/ALINX_AX7020_v1.0/adc/`）。
...
> **A2 测试方案 v2（2026-08-25 用户定）**：采集与上行解耦——**PL 环形缓冲连续采集（永不停机）**，PS 收到 UART 指令后上传**最新固定 1s 全 8 通道**原始采样值（数据量固定、非参数），传完即断开；新增**保存数据文件 + 8 通道「ADC 原始值 vs 时间」波形图**对账证据（不换算电压）。需求已改版 [B12_a2_requirement_draft.md](../tests/B12_a2_requirement_draft.md)（**v2.2**：上传纪律 + **快照冻结解耦硬性要求**——混帧截断/频率误读的根因）。D-D 从框架缺陷改判为测试协议问题（由需求确定性协议根治）；开发流程修复轮 #1/#2/#3 已落地（D1/D-B/D-E/D-A/D-C、deadline watchdog、pl_reset_run、manifest 版本化等）；白盒重跑（Agent1c）→ 白盒审查通过/保守修复后 A2 黑盒，按序自动推进（用户授权）；仅重大问题停止等用户。

> **A2 v2 白盒完成记录（2026-08-26，Agent1c，报告 [B12_a2_whitebox_v2_report.md](../tests/B12_a2_whitebox_v2_report.md)）**：project_h 全量贯通——平台/PL/bitstream/PS 三 Manifest + 一致性 12/12 + `A2_PASS`；方案乙架构 + **快照冻结（需求 v2.2）**落地；**FSCAL 板上实测 1999Hz**（fs=2000 确认，XTime=CPU/2 标定）；**完整 2000 帧零丢字节**（57600 波特率 + 分批，sum16/长度校验通过）；8 通道波形图证据完整（1s 恰 10 周期正弦、其余通道近 DC）；盲测通道与频率均由数据推导、外部独立重算一致，**与用户答案对账：通道精确命中、频率达标**（对账数值见白盒报告，黑盒禁入）。期间发现并修复：RTL 多驱动、XDC 行内注释、固件 UART API 三错、T_CONV 读时序、115200 捕获字节损坏（降 57600）。新框架 P2 已记录（D2 位流自复制、reset_run -force、捕获高波特率损坏），黑盒前修复轮 #4 处理。
