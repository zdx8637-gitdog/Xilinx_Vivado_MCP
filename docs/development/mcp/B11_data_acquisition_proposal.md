# B11 数据采集验证实例候选（DRAFT）

> 日期：2026-08-14（`Get-Date` 实测 2026-08-14 17:19 +08:00，首次成稿 16:26）
> 状态：**DRAFT — 待用户审核。本文档是「验证实例候选」文档，不代表已立项、不冻结任何资产、不修改任何生产代码 / 测试 / skills / boards。**
> **定位改述（2026-08-14，用户方向重定后）**：B11 的目标已重定为「泛化框架黑盒验证」——用 GPIO 项目当考题、Skill 完全去 GPIO 化，证明 Skill+MCP 是面向任意 Zynq 工程开发的通用框架（见 `docs/development/mcp/B11_plan.md`）。**数据采集不再是当前立项对象**，本文档降级为**验证实例候选**：其中的切片分解、带宽预算、能力缺口与流契约草案内容，保留作为未来「数据采集实例」立项时的输入材料，届时再按 B11 规划确立的「通用框架 + 领域知识包」机制重写。配套设计见 `docs/development/skill/B11_generalized_skill_design.md`（已定位为 B11 阶段①的设计基础）。
> 基线：B10 冻结包（`docs/development/mcp/B10_freeze_manifest.md`，tag `o7r3-baseline-20260813`）；唯一 MCP `mcps/zynq_mcp/`（101 工具：9 control + 92 domain，机械统计 `Tool(name=` = 101；B11 阶段②将因 platform_generate 移除变为 100）；板卡 `boards/ALINX_AX7020_v1.0/`。

## 1. 目标重述与边界（验证实例候选视角）

候选实例的用户需求（记录于 B10 清单 §6，作为未来实例输入）：

> PL AD 采集（pin 配置由用户提供）→ DMA → DDR3 → PS 读 DDR3 → UART 上行到上位机，上位机做实时成像/分析。

若未来以本候选立项，Zynq 侧职责边界（当前仅作参考，立项时按 B11 规划重新审定）：
...
边界声明：本候选只覆盖 Zynq 侧（候选子切片 B11-A/B/C）；上位机侧验证以「契约桩」（按 §6 契约解析的脚本/桩程序）在本框架内自证机读完整性，不替代上位机智能体。
...
- 本文档为 **DRAFT**（验证实例候选），待用户审核；不写 FROZEN/COMPLETE，不声称已立项。
- **定位**：B11 当前立项对象是「泛化框架黑盒验证」（`B11_plan.md`）；本候选不参与 B11 当前规划，仅作为未来「数据采集实例」立项的输入材料。若未来立项，按 B11 规划确立的「通用框架 + 领域知识包」机制重写，并以正式规划为准。
- B11-A/B/C 门禁、缺口清单、流契约草案均为候选值，立项后以正式规划为准。
