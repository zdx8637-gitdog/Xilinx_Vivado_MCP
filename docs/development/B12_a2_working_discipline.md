...
- **白盒 v2 子代理 40bcfd5c**：✅ **已放行（2026-08-25）**——S0–S8 全流程，方案乙，证据 = 数据文件 + 8 通道原始值波形图 PNG + measurement.json + A2_PASS；报告 `docs/development/tests/B12_a2_whitebox_v2_report.md`。中途 BLOCKED@S5-bitstream（XDC 行内 `#` 注释致约束全丢 → UCIO-1；根因已修复并核实）→ 二次放行后**设计已通过 bitstream（DRC 0 errors、timing met）**，但被框架重跑限制阻塞（A/B/C）→ **第三轮已放行**：全新 project_path 一次性路径（adc_top + project 下 stage RTL/XDC + 严格按序）直通 PS/S6/S7/S8；指令已含"再遇框架级阻塞即停，不再重置"。
- **新增 P2 技术债记录（黑盒前修复轮范围 = A/B/C/D，影响黑盒重试能力）**：A 无 `pl_reset_run`；B 无 stage 回退（bitstream 失败后 P7 证据丢失、不可就地重跑）；C `platform_export_manifest` 同 rev 但 generated_at/xsa_sha 变化时报 ConflictError 不可重发布；D add_ip 回读假阴性（axi_bram_ctrl/blk_mem_gen 配置，D-A 修复未覆盖）。E system_top 命名被 pl_generate_system_top 覆盖（绕行：改名 adc_top）、F XDC 行内注释（绕行：注释独占行）、G build_manifest 只搜 project/**（绕行：stage 到 project\rtl/project\xdc）。
