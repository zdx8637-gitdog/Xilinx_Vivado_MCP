...
6. **子代理通讯机制（长期纪律，2026-09-01 实测定论）**：send_message 只排队不打断；方向级指令必须「interrupt + 合并投递」；发出后必须跟进确认投递（**不能发了就不管**）；子代理 ask_user_question 父代理看不到、由**人类转述**；队列消息不可删除、用合并指令的跳过规则消化。全文见 `docs/development/subagent_communication_rules.md`（长期文档，CLAUDE.md 概览区有指针）。
...
- **CLAUDE.md 冻结规则节**哈希 `66666d2037afa7c178657c8c25c58a8addbfdabbf36731b1a1f6be232eb3e3ea`；算法 = `# AI Agent 驱动 Zynq-7020 项目规则` 标记起 → EOF 去掉尾换行。只能改标记之前的概览部分，改后必须机械复核哈希不变。⚠️ pwsh 复核哈希必须 `Get-Content -Raw -Encoding UTF8`（默认 GBK 解码会 mojibake、报 MARKER NOT FOUND 假阴性）。
...
- **★ B13 需求文档已冻结（v0.5，2026-08-29）**：`docs/development/tests/B13_requirement_draft.md`。核心链路扫描-上传模拟（WaterBear 参照简化版：只参照其"最宽 ROI 行=5000 点"帧尺寸约定，协议自定简化版）。定案：采集 = 固定 5000 行×5000 点 = 25M 点 = 50MB（CH6 单通道 16-bit 上行，2 MB/s@1MSPS，25s/图）；三档速率 2k/100k/1M；UART 下行指令（START/STOP/STATUS/SELFTEST/PING + ACK/NAK）；TCP 上行（PS=Server 192.168.1.10:5001，PC=Client 192.168.1.20，帧=MAGIC+TYPE+VER+SEQ+LEN+CRC32，一帧=一行=10KB，SEQ=行号）；DDR3 单缓冲 50MB 覆盖式；溢出=丢最旧+计数；引擎 8ch×1MSPS 能力以 L2 事件计数验收、上行仅 CH6；上位机测试程序 = 简化版（开始指令+解析显示），另派智能体。
- **★ B13 实施进展（2026-08-29 晚，双线并行）**：下位机实现子代理 d3ddc478（工作区 `D:\_b13_external\agent1_20260829\`）+ 上位机开发智能体（`D:\fpgaproject\PC_end_tcptest\`，PySide6+QML，H0-H7 计划已批）。契约补充钉死：CRC16-CCITT-FALSE **小端**（向量 0x29B1，真板正反证：正确帧 ACK/坏 CRC NAK 0x02）、CRC32 **大端**（zlib）、END=状态帧保留[0]、断连重连语义、DMA 改**官方 AXI DMA(SG)**（官方案例 31/20/19，`D:\BaiduNetdiskDownload\AX7020_2023.1\course_s2_vitis\`）、数据链 xpm_fifo_async→axis_register_slice→DMA S2MM、§7.1 下位机独立验收路径（机读接收器+DCC，不依赖 GUI）。**P0 门禁 PASS**：真板 GEM0↔PC 直连，5000 行/0 缺/0 CRC 错/0 图案错/2.11 MB/s/OVF=0，8 次连续 PASS；修掉 CRC 逐位、ACK 饥饿、窗口钳制三瓶颈（1MB 缩放窗口+分片 pace）。**P1 已放行进行中**（PL 全链 TPG 门禁：FIFO→AXI DMA SG→DDR 50MB→TCP，覆盖式验证）。mock 下位机 `.tmp_agent_tools\b13_mock_lower.py`。
- **⏱️ 3 小时硬计时（用户 2026-09-01 22:20:46 定）**：期限 **2026-09-02 01:20:46**；到时若 B13 下位机子代理 d3ddc478 仍在工作 → 强行 `interrupt_agent` + `send_message` 责令立即汇报当前进度与证据。计时器 = 后台 job `pwsh-114`（到期通知驱动动作）；若子代理在期限内先完成，收到完成通知时 `job_kill pwsh-114` 清理计时器。
- **★ B13 进展（2026-09-01）**：P0 终版 PASS（§4 契约追加 + §7.1 独立验收工具：状态帧恒 22B + END 首字节——STATUS22 自测当场抓到 put_be32 字节错位缺陷；断连重连真板 RECONNECT PASS（重连首 SEQ=3364、ovf=422=断连覆盖行、重连补发 END）；字节序；`b13_receiver.py` 机读接收器交付；复跑 2.1086 MB/s 零错）。**ACK/NAK 上行线格式已钉入主文档 §3.1**（真板实测：ACK=原命令|0x80、NAK=0x00+reason 0x01–0x07、帧后 ASCII 调试文本非帧内容、3 条真板 KAT）→ 上位机修改 prompt 已交用户转达（其 0x06/0x07 草案作废）；**上位机已收敛完成并经主代理复核**（contracts/control-contract.md §4 重写、protocol.py/mock/tests 对齐、全量 51/51 我亲自重跑 PASS）。**P1 平台路线裁决 = 方案 2 自建**：子代理曾误判「平台域无法注入自定义 RTL」并抛三选项（PS 直写/手工 Tcl/31 XSA）——主代理裁决**三选项全驳**，唯一路径 = 平台域 BD（PS7+DMA SG+slice+interconnect，已建成）+ `platform_make_external` 引出 S2MM + AXI GPIO 控制 + PL 域 `pl_create_project` 的 `sources` 参数（= add_files 通道，.bd+RTL 同注入；B12-A2 黑盒 pl_b12a2.xpr 先例）+ 自研 TPG 引擎/xpm_fifo_async 4096 RTL（已写好、xvlog 过）。**⏸️ 2026-09-01 因峰时计费暂停（用户指示）**；恢复方式 = 对 d3ddc478 `send_message` 唤醒（队列已含硬性规则与更正消息，唤醒后按裁决继续：平台域收尾→PL 域合流→部署→L1 门禁）。**硬性规则（用户定）**：子代理禁用 ask_user_question，决策点停止+汇报等主代理答复；人类转贴主代理裁决文本到答题卡=有效送达。
- **★ B13-P1 最终对账（2026-09-02，主代理定稿）**：d3ddc478 停用前遗留平行收尾曾覆盖磁盘/板卡（bit 08FDA271/XSA BC6CC4F4/ELF A7C96CE5，BUILD=b13-p1-20260902）并**回退了 RST 修复**——主代理核板发现后，在其最终源码上重新应用 RST 修复（保留其版本串+调试清理），重编 ELF sha=**8a3882d4**，xsdb 部署，真板复验：RST 回归 3/3 过 + **最终门禁 4/4 PASS、consecutive_pass=2、8.53–8.57 MB/s**。**P1 最终产物（板载）：bit 08FDA271 / XSA BC6CC4F4 / ELF 8a3882d4**；版本标识 BUILD=b13-p1-20260902/PL=b13p1pl/PS=b13p1a1。里程碑定稿 P1_MILESTONE.md §10（§7/§9 旧 sha 为过程产物）。子代理 d3ddc478 已永久停用（[ready]，勿再发消息）。P1 完全关闭；P2（ADC 三档实采+L2）待用户指示，届时**新派干净子代理**（任务提示写明禁 ask_user_question）。
- **✅ P2 主代理独立亲测完成（2026-09-03，板卡断电重启后）**：后台跑 p2_gate.py 2 → **verdict=PASS、consecutive_pass=2**（三档 1999/99908/999591、L2 1:1、conv=points、TPG 档位映射全过）；p2_rst_principal.py → **RST 回归 3/3**；REG_READBACK PASS。**P2 = 白盒 2 连 PASS + 主代理复测 2 连 PASS 双源证据**。板载 P2 最终固件（bit 1da587bc / ELF d6f94091）。恢复流程定稿：断电重启 → `reset_deploy_p2_v3.tcl`（stop→rst→ps7_init→fpga -f→[停]→ps7_init→dow→con；注意 post-fpga 的 stop 报 Already stopped 属正常，用 boot_p2_v4.tcl 续跑）。**下一步 = B13-P3 全链联调（下位机 P2 固件 + 上位机 51/51 就绪），待用户指示。**（教训已固化：长命令一律后台 job；xsdb 恢复脚本禁 catch 吞错；post-fpga 的 stop 报 Already stopped 属正常态。）
- **★ 框架修复轮#2–#5 + 分支策略（2026-09-03，用户定）**：**升级工作全部进 `framework-iteration` 分支（已推 GitHub），master 保持已验证状态，白盒+黑盒通过后再合并**。修复轮#2（M5 重试语义）= dedup 共享助手 `dedup_lookup`：仅 SUCCEEDED 阻断重试，FAILED/CANCELLED 合法重试，未决态仍由 P6 接管（+7 例）。修复轮#3（M3 XSA 确定性）= `xsa_normalize.py` 接入导出（+6 例）。修复轮#4（M4 元数据同步）= `_read_platform_manifest` 以**磁盘 platform.xsa 哈希为真相优先**（陈旧 session 修订不再选中过期 manifest；f3dcaa45 vs 22b94b0f 根治）（+1 例）。修复轮#5（M5b define 幂等）= compile_app 容忍 XSCT「already contains the item」（**真机 xsct 探测验证**；-get 静默不可用；其余错误照常失败）（+2 例）。**最新全量回归 1459 passed / 1 skipped / 41 deselected / 0 failed**。分支头 ece4dcb。**剩余唯一大项 = M2 平台原子**（platform_package_user_ip + BD 端口属性/接口宽度原子——需真实 Vivado 验证 + 真实 BD 测试，留待下一轮专用工具链时间）；Skill 四补（S1-S4）为文档层，可随时做。
- **★ 框架修复轮#6 = M2 平台原子 + Skill 四补（2026-09-04，中国标准时间）**：① `platform_package_user_ip` **两段式**：打包 = 一次性 `vivado -mode batch` 子进程（真 Vivado 实测幂等打包 ×2；**会话内 create_project 会关闭设计工程并摧毁内存 BD = P0，已用子进程回避**），注册 = 会话内非破坏追加 ip_repo_paths + update_ip_catalog + VLNV 回读（NO_OPEN_PROJECT → USER_IP_NO_OPEN_PROJECT fail-closed）；**Tcl 路径反斜杠归一化**（真机复现：未加括号的替换词会吞反斜杠 C:\Users→C:Users → get_ipdefs 空 → Common 17-161，改正斜杠根治）。② `platform_set_bd_object_property`（对象类型自动识别 port→pin→intf-pin）：真 Vivado 实测 **ASSOCIATED_BUSIF 真实家在 IP 时钟 pin（bd_ports 上不存在，BD 41-1642）**、axi_interconnect S00_AXI DATA_WIDTH 只读（CRITICAL WARNING+空回读 → BD_OBJECT_PROPERTY_VERIFY_FAILED fail-closed）。③ make_external 接口派生名经 `get_bd_intf_ports -of_objects [get_bd_intf_nets ...]` 实采（真机：S_AXI→S_AXI_0/S_AXI_1，pin 基名猜测必错；pin 直查 -of_objects 匹配空 BD 5-233）。④ Skill S1-S4 四补（5_domain_implementation S1+S2 / 7_deployment_observation S4 / appendix_mechanics S3 §12）。**证据：host_live 真 Vivado 2/2 PASS（打包幂等 + 会话并存注册→实例化闭环）；全量非硬件回归 1477 passed / 1 skipped / 43 deselected / 0 failed；collected 1521；.mcp.json SHA 不变**。工具数仍 109（11 control + 98 domain）。commit **81fa176**（framework-iteration）。**push 暂受阻：代理 127.0.0.1:7890 未运行（直连 github.com:443 亦不通），本地已提交，待代理恢复补推**。
- **★ B13-P2 白盒完成 + 主代理审核 PASS（2026-09-03）**：v4-pro 接管主会话（经 mcp_client stdio 客户端驱动 105 工具）完成 P2。**读挂根因**：P2 BD 重建后 converter M_RREADY 在 AR 后即保持 1，引擎读 FSM `RVALID<=1` 被同拍 `if(RREADY) RVALID<=0` 覆盖（last-write-wins）→ RVALID 零宽脉冲 → CPU 卡 Xil_In32（P1 BD RREADY 晚到故侥幸）。修：RVALID/BVALID 保持至握手 + AR 门控（真转换器 IP 仿真 pre 5/5 挂 → fix 5/5 通）。**门禁 2 连 PASS**：三档 1999/99915/998910 Hz（±0.5% 内）、L2 1:1（CONV=BUSYF=FRSTD=5M、RD=40M、RERR=CERR=0）、ADC-CH6 TCP 冒烟（CRC 0/SEQ 连续）、TPG 档位映射、conv 真计数；RST 回归 3/3；verify 12/12。**最终产物（主代理亲测 sha）**：bit **1da587bc**（09-03 01:15）、ELF **d6f94091**（含 RST 修复）、manifest platform 6bf2e166/PL 8ed2e8db/PS cd2be5be。板载最终 P2 固件。**审核注记（P2 不阻塞）**：① p2_gate 里 2k/100k 档 TPG 冒烟 stats verdict=FAIL 是接收器 ≥2MB/s 阈值对低档速不适用（速率实测 1999/99906 达标，agent 已 pass:true 覆盖）——P4 前把门禁脚本判据按档位区分；② manifest rev 307130c4→6bf2e166（XSA 导出非字节确定 + 结构等价 HWH/ps7_init 逐项证明）；③ 部署先 ps_halt_target 再 ps_initialize_ps（DAP 卡死教训）。**下一步 = B13-P3 全链联调**（下位机 P2 固件 + 上位机 51/51 已就绪），待用户指示。
...
2026-09-01 起全文迁移至 `docs/development/subagent_communication_rules.md`（跨 Brick 长期纪律，CLAUDE.md 概览区有指针），本文件不再重复维护。要点：send_message 只排队不打断；方向级指令「中断 + 合并投递」；发出后必须跟进确认（**不能发了就不管**）；ask_user_question 由人类转述 + 父代理裁决直投；队列消息不可删除、用合并指令的跳过规则消化；合并指令五要素（版本号/覆盖清单/重启口径/回退触发/冲突升级）。
