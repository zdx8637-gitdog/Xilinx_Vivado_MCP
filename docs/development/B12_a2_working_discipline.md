...
- **白盒 v2 子代理 40bcfd5c**：✅ **已放行（2026-08-25）**——S0–S8 全流程，方案乙，证据 = 数据文件 + 8 通道原始值波形图 PNG + measurement.json + A2_PASS；报告 `docs/development/tests/B12_a2_whitebox_v2_report.md`。**project_h 全量重跑（修正版）贯通**：FSCAL 实测=1999Hz（fs=2000 板上确认）、CH6 干净正弦（std 3166，其余 <20 LSB）、一致性 12/12、A2_PASS；频率 = 内部 FFT 10.62 / 外部四参数拟合 **9.9649Hz（0.35% ✓）**、Vpp 2.68V。**已授权有界证据强化**：固件放慢上传节奏 + ≤3 次完整 2000 帧尝试；失败则接受 753 帧前缀并记录。
