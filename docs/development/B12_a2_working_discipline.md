...
- **★ 框架修复轮#2+#3 + 分支策略（2026-09-03，用户定）**：**升级工作全部进 `framework-iteration` 分支（已推 GitHub），master 保持已验证状态，白盒+黑盒通过后再合并**。修复轮#2（B13-M5 重试语义）= P10 dedup 抽共享助手 `dedup_lookup`（execution_gate + domain_runner 共用）：仅 SUCCEEDED 终态阻断重试，FAILED/CANCELLED 合法重试（P2 外科 ledger 根治），未决态仍由 P6 门接管；+test_m5_retry 7 例；回归 1450 passed（复跑稳定；jtag_lock heartbeat 负载抖动一次，单跑+复跑均过）。修复轮#3（B13-M3 XSA 确定性）= 新增 `xsa_normalize.py`（zip 确定性重打包）接入 `platform_export_hardware` → manifest revision 只依赖内容（307130c4→6bf2e166 漂移根治）；+test_m3_determinism 6 例；**回归 1456 passed / 1 skipped / 41 deselected / 0 failed**。分支头 41dc1d2。**待续（同一分支）**：M5b compile_app define 幂等（需 XSCT 行为验证）、M2 平台原子、M4 元数据同步。
