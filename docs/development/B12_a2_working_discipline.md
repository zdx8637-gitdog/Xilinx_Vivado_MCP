...
- **白盒 v2 子代理 40bcfd5c**：✅ **已放行（2026-08-25）**——S0–S8 全流程，方案乙，证据 = 数据文件 + 8 通道原始值波形图 PNG + measurement.json + A2_PASS；报告 `docs/development/tests/B12_a2_whitebox_v2_report.md`。**第七轮（v2.2）关键发现**：①快照冻结（Option B：DDR 预读）验证成功——完整 2000 帧零丢字节、时间轴连续、454/1353 跳变消失；②**FSCAL 实测实际帧率 = 4000Hz**（cfg_div=50000 → FCLK0 实为 ~200MHz 而非配置的 100MHz，名义 2000Hz 偏 2×）——**此前 11.0086Hz 结论撤回**（基于混帧损坏数据）；③当前 ADC 读数不稳定（T_CONV_CLK=200 @200MHz = 1µs < AD7606C-16 t_CONV≈4µs → 转换未完成就读）。**已授权修复轮**：cfg_div=100000（→精确 2000Hz）+ T_CONV_CLK≥1000 + 保留快照冻结 + project_h 重跑 + FCLK0=200MHz 之谜调查（ps7_init PLL 分频/configure_ps7 是否未生效——可能又是一处"配置成功≠生效"框架缺陷）。**已向用户确认波形发生器是否仍在驱动**（当前全通道近 DC 也可能是信号源未接入）。
