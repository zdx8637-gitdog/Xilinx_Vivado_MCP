...
- **黑盒（431fa5c5）**：⏸️ **暂停（2026-08-28 白天，用户成本考虑，晚上再跑）**。已中断。**晚上恢复时的任务**（指令已排队在其消息队列）：执行 **WR=1 对照实验**（`adc_wr` 恒拉高 + 保留 v12 读状态机 → 重建部署采集）——这是当前最可能的根因（CM2368 克隆芯片"CS 低时 WR 下降沿切总线到三态"；白盒恒拉 WR=1 能读、黑盒 v6 起 WR=0 读不到）。若 WR=1 恢复数据 → 完成盲测；若仍无数据 → 下一步查模块芯片丝印（需用户看芯片标记 AD7606C vs CM2268）。黑盒历史：v5 BUSY 边沿方向、v6 厂商配方、v7 通道重复、v8 去 FRSTDATA 门控、v9 CS=RD 短接、v11 厂商逐拍照搬、v12 延迟扫描——全部真板验证、接口信号全对（fr 对齐、idle=0、FC 精确）但 DB 只有两电平伪影。注意：黑盒用过 ask_user_question 会阻塞回合（答案未回传），恢复时如再犯就中断并代答。
