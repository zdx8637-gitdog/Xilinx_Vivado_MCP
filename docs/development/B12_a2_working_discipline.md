...
- **★ 框架修复轮#1（B13-M1 环合法化，2026-09-03，主代理主理）**：新增 `workflow_rollback`/`workflow_resume_from`（control 11 工具；control/workflow.py：lane=IDLE+无 active op 校验、扩展 ROLLBACK_TARGETS（PS_BUILD→PL_BUILD 等）、下游 revision 失效、workflow_history 迭代可观测、dedup 重置；resume 仅向前+产物校验）；修 Skill 机械门禁违规（phases/5 的 GPIO 字样→纯电平直驱）。测试：+test_workflow_loop.py 14 例；6 处工具数断言 105→107。**全量非硬件回归 1442 passed / 1 skipped / 41 deselected / 0 failed（221s）**。MCP 总数 107。待续 M3/M5/M2/M4（backlog 见 B13_exposed_framework_issues.md）。
