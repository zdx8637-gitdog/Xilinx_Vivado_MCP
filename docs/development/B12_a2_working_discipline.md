...
- **★ 框架修复轮#2（B13-M5 重试语义）+ 分支策略（2026-09-03，用户定）**：**升级工作全部进 `framework-iteration` 分支（已推 GitHub），master 保持已验证状态，白盒+黑盒通过后再合并**。修复轮#2 = P10 dedup 抽共享助手 `dedup_lookup`（execution_gate + domain_runner 共用）：仅 SUCCEEDED 终态阻断重试，FAILED/CANCELLED 合法重试（P2 外科 ledger 事件根治），INTERRUPTED/TIMED_OUT/OUTCOME_UNKNOWN 仍由 P6 门接管（分层语义保留）；+test_m5_retry 7 例；**全量非硬件回归 1450 passed / 1 skipped / 41 deselected / 0 failed（复跑稳定；首跑 jtag_lock heartbeat 负载抖动一次，单跑+复跑均过）**。**待续（同一分支）**：M3 确定性构建、M5b compile_app define 幂等（需 XSCT 行为验证）、M2 平台原子、M4 元数据同步。
