...
- **黑盒（431fa5c5）**：⏸️ **暂停（2026-08-28 白天，用户成本考虑，晚上再跑）**。已中断；**v14（WR=1）对照实验在中断前跑完：WR 电平排除**——仍是两档伪影（奇≈−15.3/偶≈−4.6，RMS<1 LSB），接口信号依旧全对（frs=0x01、FC 精确、SUM32 通过）。已穷尽矩阵：RD 周期 40/80/120/200ns × 采样 20–180ns 全扫 × CS 三方案 × FRSTDATA 门控三式 × WR 0/1 × 厂商逐拍照搬。**晚上恢复的选项（待用户定）**：①主代理给黑盒"白盒控制时序配方"（CONV 40ns 脉冲→6µs 固定延时→CS 低→8×RD 约 50/10ns→采样于 RD 上升前 1 拍；RESET 从不脉冲；FR_D 不接）——与厂商例程不同但本板实测有效，需用户授权（盲测答案仍保密）；②查模块 OS 引脚实际电平（模块可能有上拉/跳帽致软件模式）；③查模块芯片丝印（AD7606C vs CM2268，需用户目视）；④查 DB15/BYTE SEL 引脚电平。黑盒历史（v5–v14 共 9 代全真板验证）已存档。
