...
| 盲测：通道号/频率自采集数据测定 | **通道号初步测定 = CH5（强证据，§8）**；**频率/Vpp 测定被阻断**（需完整流） |
...
- **通道观测**：CH0≈65518(-18)、CH1-4/6/7≈65518-65535（近 0/小偏移，悬空通道）；**CH5 明显变化**：
  ...
  峰峰值初步 ≥2.38V）。**有信号通道 = CH5（第 5 通道）**，识别依据 = 全 8 通道数据。
...
- **测量结论（初步，非最终）**：
  - 通道号：**CH5**（第 5 通道，证据 = 全 8 通道方差/幅度对比，仅 CH5 大幅变化）。
...
- **盲测频率/Vpp（最终值）**：采样流被 D-D 阻断，未能测定；仅通道号（CH5）与 Vpp 初步估算。
