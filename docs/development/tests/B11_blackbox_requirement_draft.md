...
- 行为：模式 A 与模式 B **交替**，每个模式保持约 1 秒（可观察的闪烁节奏），
  **持续循环不停止**：满 8 轮（A→B 计 1 轮；即模式 A、B 各出现 ≥ 8 次）且全部
  读回一致后打印一次 `LED_E2E_PASS`，随后**继续无限交替**（约 1s/模式），直到
  复位或断电；任一不一致打印 `LED_E2E_FAIL` 并停止（见 §3.3）。

### 3.2 读回要求（每个 LED 的证据）

- **每个 LED 必须提供读回证据**：每轮输出当前模式的实际写入值与读回值。
- 读回语义：PL 侧 LED 经 PL 地址空间读回（如 GPIO 数据寄存器）；PS 侧 LED 经
  MIO GPIO 数据寄存器读回。读回值必须与写入值**逐位一致**（含 active-low
  语义——写入 0 的 LED 读回必须为 0，写入 1 的读回必须为 1）。
- **硬约束（读回真实性）**：读回必须来自**引脚/外设真实状态寄存器**（如 PS 端
  DATA_RO 类只读输入状态寄存器，反映引脚物理电平），**不得读写入镜像/写侧
  寄存器**（写镜像回读恒等于刚写入的值，会产生「自洽假象」——UART 全对但
  引脚实际未动）。读回值必须能反映方向/输出使能配置后的实际引脚电平。
...
| 条件 | 输出 | 判定 |
|---|---|---|
| 满 8 轮全部通过（每轮 6 位写入 == 读回） | `LED_E2E_PASS`（打印一次；程序**继续**无限交替运行） | PASS |
...
- 超时建议：程序启动后 60 秒内未收到 PASS/FAIL marker 判 TIMEOUT（观测端）。
  **注意**：PASS 打印后程序不会退出（持续交替至复位/断电），观测端捕获到
  PASS marker 即可判定并停止捕获，无需等待程序结束。
...
- **物理确认由用户阶段⑤执行**：6 个 LED 全部点亮模式可观察（含 2 个 PS LED
  也必须亮——B11 阶段③曾出现 UART 读回全对但 PS LED 物理不亮，属读回镜像
  自洽假象，见 §3.2 硬约束）。
