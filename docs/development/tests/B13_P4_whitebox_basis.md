...
- **旧白盒参照（只读，永不修改）**：`D:\_b13_external\agent1_20260829\`（P0/P1/P2 全过；板载 P2 最终固件 bit 1da587bc / ELF d6f94091；含 mcp_client.py/mcp_call.py 工具与全部证据）。
  - **白盒可读取引用**（白盒 = 全知实现方，同一 Agent1 线延续，源码/设计/证据可作参照）；**仅禁止写入/覆盖/删除**。
  - **证据必须新生成**：白盒验收证据须在新工作区 `evidence\` 重新产出，禁止用旧证据顶替。
  - **框架机制清单（§五）证据必须在新框架上真实执行取得**：M1–M5 的证明不得搬旧产物；产品链 RTL/固件源码可继承 P0–P2，但须在新框架重建并重新取证。
...
- 硬件操作严格串行；长命令后台执行。
- 冻结资产与旧白盒工作区只读（**可读引用、不可修改**；白盒证据须在新工作区新生成）。
