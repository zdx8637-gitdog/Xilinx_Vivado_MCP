# B12-A2 黑盒验收基线（FROZEN — 2026-08-26；2026-08-29 修订记录见 §7.5）
...
1. 供给材料 SHA256（冻结值 v2，2026-08-27 公开事实勘误后实测）：
   ...
   - 模块事实卡（勘误后）：`2caded5472c821e47306d341afb08ab718789434f79c5030e6ef664b5d9b8de2`
   - 引脚 JSON（FR_D 方向勘误后）：`a8fd6f8ffc3da93d72163c0d2e742b4daa6ccd6408d9c2323119b2349f50c44b`
...
## 7.5 修订记录（2026-08-29，重开黑盒前）

> 依据 §5 联动条款：泛化 Skill 测试纪律合并（可测性设计 5.4 / 自检阶梯 7e /
> S8 故障归因约束 / 附录 10-11，经用户多轮评审定稿）改变了黑盒执行面的
> Skill 供给材料，须同步更新基线 SHA 后再派发。变更性质：新增通用测试纪律
> 章节，不改变 S0–S8 既有流程语义；对黑盒的影响 = 实现须内嵌自验证设施并按
> 7e 走 L1→L2→L3 阶梯。

1. **重开模式**：照 A1/A2「全新无记忆 + 隔离区 + 仅公开契约」重开；旧黑盒智能体
   （其隔离区 `agent3_20260825`）已停止并弃用，**新隔离区 =
   `D:\_b12_a2_external\agent3_final_20260829\`**（全新空目录）。
2. **供给材料 SHA256 更新（本次唯一变更 = Skill 五个文件；其余冻结值不变）**：
   - SKILL.md：`38F8689C8EA5CDD88041AC86FA2AB5FE4BC818F5C901E1E9D0250A84F04755A2`
   - phases/5_domain_implementation.md：`9FF2A6254173D566F16F4D20F0AE9997BC5A794CF78880468A58796F8968FBE4`
   - phases/7_deployment_observation.md：`96964D35A600C5125555ADAE80FDD3A77421A1C5851648CA1B5D355181B6A238`
   - phases/8_verdict_recovery.md：`88C7053ED4754B26E7B90ED234472B7E994786411ED5B28B2129B4FE6AB0FB10`
   - appendix_mechanics.md：`E92B6B4032A9F029D902BADBE70615CB78DB49C29F417121BA96DE55AA7A3873`
3. 其余供给（需求 v2.2 / board_profile / package_manifest / 模块事实卡 / 引脚
   JSON）SHA 未变，沿用 §7.1 冻结值。
