...
白盒任务书必含：① 目标（产品判据+机制清单）；② L0 动态对拍项（每簇抽 1–2 代表工具）；③ **L2-A 强制转移清单** + **L2-B 本 Brick 扰动清单（有界）**；④ L3 规则（能力簇覆盖 + 诊断回退白名单）；⑤ 偏离审计义务（auditor 输出 + 结构化记录 + A–F 归类）；⑥ 专家帮助率统计义务；⑦ 输入白名单 SHA + 隔离纪律；⑧ 完成标准 + 停止汇报纪律。
