...
| `mcps/zynq_mcp/` | 唯一 MCP（共 105 工具：9 control + 96 domain；B12-N3 新增 `ps_start_hw_server`，见 [B12_n3_hwserver_tool_report.md](docs/development/mcp/B12_n3_hwserver_tool_report.md)；修复轮#2 新增 `pl_reset_run`）：`control/`（execution_ledger、single_worker、execution_gate、instance_guard、process_guard、session、recovery、operation_service、operation_registry、capabilities、context、timeout_config、workspace）、`adapters/`（vivado/xsct/jtag/uart）、`domains/`（pl/platform/ps）。B10 已知限制①（计数漂移）已由 B11 阶段②关闭：`DOMAIN_APIS_IMPLEMENTED` 机械派生 |
