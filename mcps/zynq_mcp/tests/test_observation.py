...
Covers evaluate_observation verdict rules (FROZEN):
...
through the production query path. Pure text analysis — no hardware, no board.
"""
...
class TestObservationVerdicts:
    ...
    async def test_pass_marker_verdict_pass(self):
        result = await evaluate_observation(uart_text=f"boot ok\n{PASS}\n")
    ...
    async def test_fail_marker_verdict_fail(self):
        result = await evaluate_observation(uart_text=f"something broke\n{FAIL}\n")
        assert result["status"] == "success"
        d = result["data"]
    ...
    async def test_empty_text_timeout(self):
        result = await evaluate_observation(uart_text="")
        assert result["status"] == "success"
        d = result["data"]
    ...
    async def test_truncated_pass_frame_is_incomplete(self):
        ...
        result = await evaluate_observation(uart_text="...GPIO_E2E_PAS")
        d = result["data"]
        assert d["verdict"] == "INCOMPLETE"
        assert "GPIO_E2E_PAS" in d["partial_markers"]
        assert d["pass_marker_found"] is False

    @pytest.mark.asyncio
    async def test_truncated_fail_frame_reported(self):
        result = await evaluate_observation(uart_text="...GPIO_E2E_FAI")
        d = result["data"]
        assert d["verdict"] == "INCOMPLETE"
        assert "GPIO_E2E_FAI" in d["partial_markers"]

    @pytest.mark.asyncio
    async def test_preview_capped_at_200(self):
        long_text = "A" * 500 + PASS
        result = await evaluate_observation(uart_text=long_text)
        d = result["data"]
    ...
    async def test_non_string_text_errors(self):
        result = await evaluate_observation(uart_text=12345)
    ...
    async def test_bridge_accepted_but_unused(self):
        ...
        result = await evaluate_observation(object(), uart_text=f"ok {PASS}")
        assert result["status"] == "success"
        assert result["data"]["verdict"] == "PASS"
    ...
    async def test_idempotent(self):
        ...
        text = f"boot\n{PASS}\n"
        r1 = await evaluate_observation(uart_text=text)
        r2 = await evaluate_observation(uart_text=text)
...
class TestQueryToolRegistration:
    ...
    def test_schema_registered(self):
        ...
        assert t.inputSchema["required"] == ["uart_text"]
