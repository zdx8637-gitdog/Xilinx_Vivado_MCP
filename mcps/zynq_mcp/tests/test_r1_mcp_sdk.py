...
class TestAllTools:
    def test_tool_count_matches_capabilities(self, zynq_env):
        """E007: total_tools=104 after B11 ③.1 (platform_generate removed in
        phase 2, then assign_addresses/make_external/synthesize added):
        9 control + 95 domain. The full composition chain: R3.1-C 10 → B05
        ...
        async def _run():
            async with stdio_client(params) as (r,w):
                async with ClientSession(r,w) as s:
                    await s.initialize()
                    d=await _call(s,"get_capabilities"); assert d["data"]["total_tools"]==104
