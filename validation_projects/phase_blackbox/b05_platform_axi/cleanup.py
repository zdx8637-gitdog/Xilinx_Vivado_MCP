"""Cleanup script for B05 Platform/AXI smoke runner evidence. v1.0.0
...
def _validate_run_id(rid):
    if not isinstance(rid, str) or not rid: return False
    if rid in (".",".."): return False
    if "/" in rid or "\\" in rid: return False
    if os.path.isabs(rid): return False
    if not _VALID_RUN_ID.match(rid): return False
    return True


def main():
    p = argparse.ArgumentParser(description="B05 Cleanup")
    p.add_argument("--run-id", required=True)
    p.add_argument("--execute", action="store_true")
    args = p.parse_args()

    if not _validate_run_id(args.run_id):
        print(f"ERROR: invalid run_id", file=sys.stderr); sys.exit(1)

    target = os.path.join(HERE, "evidence", args.run_id)
    if not os.path.isdir(target):
        print(f"Not found: {target}"); sys.exit(0)
    ...
    if args.execute:
        for item in items:
            if os.path.isdir(item): shutil.rmtree(item, ignore_errors=False)
